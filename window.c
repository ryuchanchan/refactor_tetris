...
/*   Updated: 2022/08/07 01:37:29 by hitoda           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void print_current_game(t_game *game, t_block current)
{
	int i, j;
	char current_block;
	...
	for(i = 0; i < MAP_HEIGHT; i++){
		for(j = 0; j < MAP_WIDTH ; j++){
			current_block = check_current_block_position(i, j, current);
...
void print_window(t_game *game, t_block *current)
{
	...
	for(i = 0; i < MAP_WIDTH - 9; i++)
		printw(" ");
	printw("42 Tetris\n");
	print_current_game(game, current);
	printw("\nScore: %d\n", game->score);
}
